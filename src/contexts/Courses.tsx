...
export const CoursesProvider = ({ children }: { children: ReactNode }) => {
    ...
    const getNotAddedCardsList = () => {
        if (userId) {
            setNotAddedCards(cards.filter((course) => !Object.keys(course.users).includes(userId)));
            setIsLoading(false)
            console.log(cards)
    ...
    return (
        <CoursesContext.Provider value={{ cards, isLoading, getCoursesList, getNotAddedCardsList, notAddedCards }}>
            {children}
